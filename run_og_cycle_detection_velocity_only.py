"""
OG/PO クラスタ間の遷移サイクルを検出するスクリプト (Velocityのみ)

- E17_og_exact_velocity.h5ad を読み込み
- CellRank の VelocityKernel のみで遷移確率行列を構成
- OG/PO クラスタ間の正味遷移を集計
...
print("Building CellRank VelocityKernel transition matrix...")
...
# サイクルの可視化
outdir = "cycle_detection_velocity_only"
...
plt.title("OG/PO cluster transition cycles (Velocity only) and edge strengths")
plt.axis("off")
plot_path = os.path.join(outdir, "cluster_cycle_strengths_velocity_only.png")
plt.tight_layout()
