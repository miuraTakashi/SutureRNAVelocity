...
for h5ad_path in h5ad_files:
    ...
    if "velocity_umap" not in adata.obsm:
        ...
        try:
            ...
            # Compute neighbors with Scanpy first (required for scVelo 0.3.4+)
            sc.pp.neighbors(adata, n_pcs=min(30, adata.obsm['X_pca'].shape[1]) if 'X_pca' in adata.obsm else None)
            ...
            # Try to compute velocity graph - if it fails, compute velocity_umap directly
            try:
                scv.tl.velocity_graph(adata)
                print("  Velocity graph computed")
            except Exception as e:
                print(f"  Velocity graph failed: {e}")
                # Try to compute velocity_umap directly
                try:
                    scv.tl.velocity_embedding(adata, basis="umap")
                    print("  Velocity embedding computed directly")
                except Exception as e2:
                    print(f"  Velocity embedding also failed: {e2}")
                    continue
