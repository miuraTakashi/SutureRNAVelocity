...
def sanitize_h5ad_for_anndata(path: Path):
    print("  - Sanitizing legacy H5AD for anndata compatibility.")
    with h5py.File(path, "r+") as h5file:
        if "uns" in h5file and "log1p" in h5file["uns"]:
            try:
                del h5file["uns"]["log1p"]
                print("  - Removed unsupported uns/log1p from H5AD.")
            except Exception as exc:
                print("  - Warning: failed to remove uns/log1p:", exc)


def read_h5ad_with_legacy_support(path: Path):
    try:
        return sc.read_h5ad(str(path))
    except Exception as exc:
        print("  - Initial scanpy.read_h5ad() failed; attempting legacy sanitization.")
        traceback.print_exc()
        sanitize_h5ad_for_anndata(path)
        return sc.read_h5ad(str(path))
...
def process_suture_file(input_path: Path, output_dir: Path, cores: int, cluster_column: str = None):
    ...
    adata = read_h5ad_with_legacy_support(input_path)
